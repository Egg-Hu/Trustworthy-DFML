...
class BiDFMKD(nn.Module):
    def __init__(self,args):
        super(BiDFMKD, self).__init__()
        self.args=args
        #file
    ...
    def train_once(self,task_id=None):
        ...
        if task_id==0 or ((task_id) //self.args.episode_batch)%self.args.generate_interval==0:
            #generate support
            teacher, specific, acc,teacher_dataset,_=get_random_teacher(args=self.args)
            teacher.eval()
            ...
            #loss_outer = F.kl_div(F.softmax(s_logits, dim=-1), F.softmax(t_logits.detach(), dim=-1))
        else:
