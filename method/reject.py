...
from black_box_tool import set_maml, get_model, get_random_teacher, kldiv, data2supportquery, \
    compute_confidence_interval, get_dataloader, Timer,shuffle_task, bias , construct_model_pool,dataset_classnum,get_teacher_sequence,get_teacher_from_sequence
...
class Reject(nn.Module):
    def __init__(self,args):
        ...
        if (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.defense ==False):
            self.checkpoints_path = './checkpoints_reject1/'+'ATTACK/'+ feature1 + '/' + feature2
        elif (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.defense ==True):
            self.checkpoints_path = './checkpoints_reject1/' + 'DEFENSE/' + feature1 + '/' + feature2
        elif self.args.oodP==0 or self.args.fakeP==0:
            self.checkpoints_path = './checkpoints_reject1/' + 'SAFE/' + feature1 + '/' + feature2
        ...
        if (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.scheduleScheduler ==False and self.args.scheduleWeight==False):
            datapool_path = self.datapool_path = './datapool_reject1/' + 'ATTACK/' + os.path.join(feature1, feature2)
        elif (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.scheduleScheduler ==True or self.args.scheduleWeight==True):
            datapool_path = self.datapool_path = './datapool_reject1/' + 'SCHEDULE/' + os.path.join(feature1, feature2)
        elif self.args.oodP==0 or self.args.fakeP==0:
            datapool_path = self.datapool_path = './datapool_reject1/' + 'SAFE/' + os.path.join(feature1, feature2)
        ...
        self.specific_state=defaultdict(list)
        ...
        # if os.path.exists('./model_pool_data_reject1/' + os.path.join(feature1, feature2)):
        #     self.model_pool_data = torch.load(
        #         './model_pool_data_reject1/' + os.path.join(feature1, feature2) + '/model_pool_data.pth')
        # else:
        ...
        #         os.makedirs('./model_pool_data_reject1/' + os.path.join(feature1, feature2), exist_ok=True)
        #         torch.save(self.model_pool_data,
        #                    './model_pool_data_reject1/' + os.path.join(feature1, feature2) + '/model_pool_data.pth')
            ...
            for it_id in range(1, self.args.episode_train//self.args.episode_batch + 1):
                if new_model==True:
                    ...
                    for _ in range(self.args.episode_batch):
                        ...
                        teacher=self.get_teacher_from_pool(t_id)
                        ...
                        if self.args.defense:
                            ...
                            new_specific=[]
                            new_rel=[]
                            memory_data_list=self.synthesizer.get_specific_data(specific)
                            for specific_id,memory_data in enumerate(memory_data_list):
                                if memory_data==None:
                                    self.specific_state[specific[specific_id]].append('accept')
                                    new_specific.append(specific[specific_id])
                                    new_rel.append(specific_id)
                                else:
                                    memory_data=memory_data.cuda(self.args.device)
                                    memory_label_rel= torch.LongTensor([specific_id]*len(memory_data)).cuda(self.args.device)
                                    with torch.no_grad():
                                        correct = 0
                                        total = 0
                                        memory_label_logits=teacher(memory_data)
                                        prediction = torch.max(memory_label_logits, 1)[1]
                                        correct = correct + (prediction.cpu() == memory_label_rel.cpu()).sum()
                                        total = total + len(memory_label_rel)
                                        acc = 1.0 * correct / total * 100.0
                                    if acc>=50:
                                        if self.model_pool_attribute[t_id]==1 or self.model_pool_attribute[t_id]==2:
                                            print('wrong accetp acc:', acc, 'model_id:', t_id)
                                        new_specific.append(specific[specific_id])
                                        new_rel.append(specific_id)
                                        self.specific_state[specific[specific_id]].append('accept')
                                    else:
                                        if self.model_pool_attribute[t_id]==0:
                                            print('wrong reject acc:', acc, 'model_id:', t_id)
                                        self.specific_state[specific[specific_id]].append('reject')
                            #check state
                            for s,state_list in self.specific_state.items():
                                if len(state_list)>=3 and (state_list[-1]=='reject' and state_list[-2]=='reject' and state_list[-3]=='reject'):
                                    self.specific_state[s]=[]
                                    self.synthesizer.data_pool.clear_specific(s)

                        else:
                            new_specific=specific
                        #add to memory
                        if len(new_specific)!=0:
                            self.synthesizer.teacher = teacher
                            self.synthesizer.c_abs_list=new_specific


                            support_query_tensor = self.synthesizer.synthesize(targets=torch.LongTensor(
                                (list(range(len(new_specific)))) * (self.args.num_sup_kd + self.args.num_qur_kd)),
                                                                               student=None, mode='all',
                                                                               c_num=len(new_specific))
                            support_query = self.transform_no_toTensor(support_query_tensor)
                            support_query_label_relative = torch.LongTensor(
                                (list(range(len(new_specific)))) * (self.args.num_sup_kd + self.args.num_qur_kd)).cuda(
                                self.args.device)
                            support=support_query[:len(new_specific)*self.args.num_sup_kd]
                            query=support_query[len(new_specific)*self.args.num_sup_kd:len(new_specific)*self.args.num_sup_kd+len(new_specific)*(self.args.num_sup_kd+self.args.num_qur_kd)]
                            support_label_relative=support_query_label_relative[:len(new_specific)*self.args.num_sup_kd]
                            query_label_relative=support_query_label_relative[len(new_specific)*self.args.num_sup_kd:len(new_specific)*self.args.num_sup_kd+len(new_specific)*(self.args.num_sup_kd+self.args.num_qur_kd)]
                            ...
                            # for c_s in new_specific:
                            #     self.synthesizer.add_pool(imgs=self.model_pool_data[t_id][],label_specifics=, c_spe_list=[c_s],mode='all')

                            task_losses.append(self.tmeta_loss(teacher=teacher,support=support,support_label=support_label_relative,query=query,query_label=query_label_relative,way=len(new_specific)))
                    if len(task_losses)!=0:
                        meta_batch_loss = torch.mean(torch.stack(task_losses))
                    else:
                        meta_batch_loss = None
                    new_model=False
                else:# memory
                    memory_losses = []
                    for _ in range(self.args.episode_batch):
                ...
                if it_id < 1000:  # 100+20
                    self.args.generate_interval = 1
                elif it_id >= 1000 and it_id <= 2000:#100+20
                    self.args.generate_interval = 200
                elif it_id>2000 and it_id<=10000:#80
                    self.args.generate_interval=200
                elif it_id>10000 and it_id<=20000:#50
                    self.args.generate_interval=200
                elif it_id>20000:
                    self.args.generate_interval = 200
