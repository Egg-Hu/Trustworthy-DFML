...
class Online(nn.Module):
    def __init__(self,args):
        ...
        if (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.defense ==False):
            self.checkpoints_path = './checkpoints_online1/'+'ATTACK/'+ feature1 + '/' + feature2
        elif (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.defense ==True):
            self.checkpoints_path = './checkpoints_online1/' + 'DEFENSE/' + feature1 + '/' + feature2
        elif self.args.oodP==0 or self.args.fakeP==0:
            self.checkpoints_path = './checkpoints_online1/' + 'SAFE/' + feature1 + '/' + feature2
        ...
        if (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.scheduleScheduler ==False and self.args.scheduleWeight==False):
            datapool_path = self.datapool_path = './datapool_online1/' + 'ATTACK/' + os.path.join(feature1, feature2)
        elif (self.args.oodP!=0 or self.args.fakeP!=0) and (self.args.scheduleScheduler ==True or self.args.scheduleWeight==True):
            datapool_path = self.datapool_path = './datapool_online1/' + 'SCHEDULE/' + os.path.join(feature1, feature2)
        elif self.args.oodP==0 or self.args.fakeP==0:
            datapool_path = self.datapool_path = './datapool_online1/' + 'SAFE/' + os.path.join(feature1, feature2)
    ...
    def train_loop(self):
        ...
        with SummaryWriter(self.writer_path) as writer:
            ...
            e_maxValAcc=None
            new_model=True
            model_id_list = get_teacher_sequence(args=self.args)
            print(model_id_list)
            timestep=0
            for it_id in range(1, self.args.episode_train//self.args.episode_batch + 1):
                if new_model==True and timestep<len(model_id_list):
                    task_losses = []
                    for _ in range(self.args.episode_batch):
                        # generate support
                        if timestep>=len(model_id_list):
                            break
                        teacher,specific=get_teacher_from_sequence(args=self.args,model_id_list=model_id_list,timestep=timestep)
                        print(specific)
                        timestep=timestep+1
                        teacher.eval()
                        ...
                        if self.args.defense:
                            ...
                            for specific_id,memory_data in enumerate(memory_data_list):
                                if memory_data==None:
                                    ...
                                    if acc>=50:
                                        if model_id_list[timestep-1]>=100:
                                            print('wrong accetp acc:', acc, 'model_id:', model_id_list[timestep - 1])
                                        new_specific.append(specific[specific_id])
                                        self.specific_state[specific[specific_id]].append('accept')
                                    else:
                                        if model_id_list[timestep-1]<100:
                                            print('wrong reject acc:', acc, 'model_id:', model_id_list[timestep - 1])
