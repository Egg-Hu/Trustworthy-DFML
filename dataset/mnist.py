...
# SPLIT_PATH = osp.join('/home/hzx/fcil/dfmeta/DFL2Ldata/mnist/split')
script_dir = os.path.dirname(os.path.abspath(__file__)) 
parent_dir = os.path.dirname(script_dir)
SPLIT_PATH = os.path.join(parent_dir, 'DFL2Ldata/mnist/split')
...
class Mnist(Dataset):
    ...
    def __init__(self, setname, augment=False,noTransform=False):
        ...
        if augment and setname == 'meta_train':
            transforms_list = [
                ...
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.13066, 0.13066, 0.13066], std=[0.30131, 0.30131, 0.30131])
                ]
        else:
            transforms_list = [
                lambda x: x.resize((self.img_size, self.img_size), resample=Image.LANCZOS),
    ...
    def __getitem__(self, i):
        ...
        return image, label
class Mnist_Specific(Dataset):
    """ Usage:
    """

    def __init__(self, setname, specific=None, augment=False,noTransform=False):#setname='meta_train' for train, else for test
        csv_path = osp.join(SPLIT_PATH, setname + '.csv')
        self.data, self.label = self.parse_csv(csv_path, setname)
        self.num_class = len(set(self.label))
