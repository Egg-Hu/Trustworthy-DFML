...
class MetaQDA_MAP(nn.Module):
    def __init__(self, args,device):
        ...
        self.device=device
        self.triu_mask = torch.triu(torch.ones(self.x_dim, self.x_dim), diagonal=1).t().to(self.device)
    ...
    def regularize(self, sigma):
        return (1-self.reg_param) * sigma + self.reg_param * torch.eye(self.x_dim).to(self.device)
...
class MetaQDA_FB(nn.Module):
    def __init__(self, args,device):
        ...
        self.device=device
        self.triu_lower_mask = torch.triu(torch.ones(self.feature_dim, self.feature_dim), diagonal=1).t().to(self.device)
    ...
    def regularize(self, sigma):
        return (1-self.reg_param) * sigma + self.reg_param * torch.eye(self.feature_dim).to(self.device)
